...
---TABLA USUARIO --Esta tabla no contiene datos en tabla maestra. Se debe incorporar los usuarios que dice el enunciado
...
---TABLA HOTEL_CIERRE --->Esta tabla no se carga ccon datos de la tabla maestra. Se comienza a utilizar desde la aplicacion
...
---Consulta de verificación, existe una reserva por persona, los codigos de reserva se repiten pero lo que cambia son los consumibles en cada registro
SELECT Reserva_Codigo,Reserva_Cant_Noches,Cliente_Nombre,Cliente_Apellido 
FROM gd_esquema.Maestra 
GROUP BY Reserva_Codigo,Reserva_Cant_Noches,Cliente_Nombre,Cliente_Apellido
ORDER BY Reserva_Codigo ;---100740

----INSERT

--SE DEBE AJUSTAR EL ESTADO PARA QUE HAGA UNA ESPECIE DE DECODE PARA EL ESTADO, DE TODOS MODOS SE PUEDE UPDATEAR DESP EN VEZ DE HACERLO EN EL INSERt,
--Ver lo puesto en estrategia
SELECT DISTINCT (m.Reserva_Codigo),
null as RESERVA_FECHA,
 M.Reserva_Fecha_Inicio,
 DATEADD(DAY,M.Reserva_Cant_Noches,M.Reserva_Fecha_Inicio) as RESERVA_FECHA_HASTA, 
(SELECT R.REG_ID 
	FROM CONTROL_ZETA.REGIMEN R 
	WHERE R.REG_DESCRIPCION=M.Regimen_Descripcion) AS RESERVA_ID_REGIMEN, 
(SELECT HOTEL_ID 
     FROM CONTROL_ZETA.HOTEL H, 
          CONTROL_ZETA.LOCALIDAD L
      WHERE HOTEL_CALLE = M.Hotel_Calle
        AND HOTEL_NRO_CALLE = M.Hotel_Nro_Calle
        AND HOTEL_ID_CIUDAD = L.LOC_ID
        AND L.LOC_DETALLE = M.Hotel_Ciudad) AS RESERVA_ID_HOTEL,
 NULL as RESERVA_ESTADO, 
 (SELECT C.CLIENTE_ID 
	FROM CONTROL_ZETA.CLIENTE C 
		WHERE C.CLIENTE_NOMBRE=UPPER(M.Cliente_Apellido + ', ' + M.Cliente_Nombre  ) ) AS RESERVA_CLIENTE
		
---RESERVAS SIN FACTURA:
select  reserva_codigo from gd_esquema.Maestra m 
where m.Reserva_Codigo not in(select DISTINCT (reserva_codigo) from gd_esquema.Maestra where factura_nro is not null);		

GO
---TABLA ESTADIA --Se deberia cargar al hacer el update para las reservas que tienen factura, podría ser un lindo trigger
