...
--TABLA ITEM_FACTURA 
 INSERT INTO CONTROL_ZETA.ITEM_FACTURA (FACTURA_NRO, ITEM_FACTURA_CANTIDAD, ITEM_FACTURA_MONTO, ITEM_DESCRIPCION)
  SELECT  M.FACTURA_NRO, M.ITEM_FACTURA_CANTIDAD ITEM_FACTURA_CANTIDAD, M.ITEM_FACTURA_MONTO, 'COSTO POR ESTADIA' 
 FROM GD_ESQUEMA.MAESTRA M
 WHERE M.FACTURA_NRO IS NOT NULL AND ITEM_FACTURA_CANTIDAD IS NOT NULL 
 AND Consumible_Codigo IS  NULL 
  UNION all
 SELECT  M.FACTURA_NRO,  1 ITEM_FACTURA_CANTIDAD, M.Consumible_Precio as ITEM_FACTURA_MONTO, M.Consumible_Descripcion
 FROM GD_ESQUEMA.MAESTRA M
 WHERE M.FACTURA_NRO IS NOT NULL AND ITEM_FACTURA_CANTIDAD IS NOT NULL 
   AND  Consumible_Codigo IS NOT NULL
 UNION all
 SELECT  M.FACTURA_NRO,  count(Consumible_Codigo) ITEM_FACTURA_CANTIDAD, 
 -M.Factura_Total as ITEM_FACTURA_MONTO, 'DESCUENTO POR REGIMEN DE ESTADIA'
 FROM GD_ESQUEMA.MAESTRA M
 WHERE M.FACTURA_NRO IS NOT NULL AND ITEM_FACTURA_CANTIDAD IS NOT NULL
...
INSERT INTO CONTROL_ZETA.USUARIO(USR_USERNAME,USR_PASS,USR_ESTADO) 
VALUES
('ADMIN','6aa66e0cd8a37b75fb80cae6afa4cac29e48035de131788f2aaf0337cb948e8a','H');
